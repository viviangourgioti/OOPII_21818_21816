	    ...
	    JButton okButton = new JButton("OK");
	   	okButton.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) { 
				...
				//Ερώτημα 2 (lecture 11)
				
				/*AllTravellers.add(new Traveller("George",20,1,1,1,0,1,0,0,"Clear",38.92,21.78,"london"));
				AllTravellers.add(new Traveller("vivian",29,0,1,0,1,1,0,0,"Clouds",38.92,21.78,"ios"));
				
				for(int i=0;i<AllTravellers.size();i++) {
					System.out.println(AllTravellers.get(i));
				}*/
				
				//candidate Traveller
				Traveller traveller = new Traveller("maria",29,0,1,0,1,1,0,0,"Clouds",38.92,21.78,null);
				int[] candCriteria= {0,1,0,1,1,0,0};
				
				Optional<Recommend> reCity=AllTravellers.stream().map(i->new Recommend(i.getRecommendedCity(),innerDot(Traveller.criteria(i),candCriteria))).max(Comparator.comparingInt(Recommend::getRank));
				System.out.println("The Recommended City:"+reCity.get().getCity());
